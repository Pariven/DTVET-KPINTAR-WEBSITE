...
function PaymentsContent() {
  ...
  // Show info toast if redirected from payment success page
  useEffect(() => {
    if (!mounted) return;
    // Check for redirect from payment success page
    const fromSuccess = searchParams.get('from_success_page') === 'true' || searchParams.get('redirected_from') === 'success_page';
    if (fromSuccess) {
  toast.info('Our customer service team will contact you within 24 hours to help you access the programme you purchased.', {
        duration: 8000,
      });
      // Remove the param from URL so it does not repeat
      setTimeout(() => {
        const url = new URL(window.location.href);
        url.searchParams.delete('from_success_page');
                  ...
                  <Card key={payment.id} className="bg-white/5 border-white/10 hover:bg-white/10 transition-colors">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        ...
                        {payment.status === 'COMPLETED' && (
                          <div className="text-yellow-200 bg-yellow-900/40 rounded-lg px-4 py-3 text-sm font-medium">
                            Our customer service team will contact you within 24 hours to help you access the programme you purchased.
                          </div>
                        )}
